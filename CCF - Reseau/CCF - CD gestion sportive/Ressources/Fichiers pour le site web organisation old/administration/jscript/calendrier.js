...
// donne le numéro ISO de la semaine  de 1 à 53
//( algo sur http://www.phys.uu.nl/~vgent/calendar/isocalendar.htm)
Date.prototype.getWeek = function() {
  ...
  wday=((wday+6)%7)+1; // les jours de la semaine seront numérotés de 1 à 7
...
function ds_template_main_above(m,y) {
	...
	code += '<tr>';
	code += '<td colspan="8" class="ds_titre">' + ds_monthnames[m - 1] + ' ' + y + '</td>';
	code += '</tr>';
	code += '<tr>';
      return code;
}
...
function ds_template_new_week(n_iso) {
        return '</tr><tr><td class="ds_cell_iso">'+n_iso+'</td>';
}
...
function ds_draw_calendar(jo, m, y) {
	...
	ds_echo('<td class="ds_subhead" style="width:10px">S</td>');
        for (i = 0; i < 7; i ++) {
	...
	ds_dc_date.setDate(1);
        no_iso = ds_dc_date.getWeek();  // détermine le numéro de la semaine
	...
	ds_echo (ds_template_new_week(no_iso));
	...
	for (i = 0; i < ds_nb_jours(m,y); i ++) {
		...
		if (j == 1 && !first_loop) {
			// nouvelle semaine !!
                        no_iso = new Date(y, m-1, i+1).getWeek(); // numero iso de la nouvelle semaine
			ds_echo (ds_template_new_week(no_iso));
		}
